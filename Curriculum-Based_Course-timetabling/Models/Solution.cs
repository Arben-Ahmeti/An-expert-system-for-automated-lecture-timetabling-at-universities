...
namespace Curriculum_Based_Course_timetabling.Models
{
    class Solution
    {
        ...
        internal bool IsTabu(string course)
        {
            //int vtabL = VT.Length;
            for (int i = ILS.lengthTL - 2; i >= 0; i--)
            {
                if (course == ILS.TabuListTS[i])
                {
                    for (int j = i + 1; j < ILS.lengthTL - 1; j++)
                    {
                        ILS.TabuListTS[j] = ILS.TabuListTS[j + 1];
                    }
                    return true;
                }
                else { ILS.TabuListTS[i + 1] = ILS.TabuListTS[i]; }
            }
            ILS.TabuListTS[0] = course;
            return false;
        }

        internal bool IsTabuSR(string course)
        {
            //int vtabL = VT.Length;
            for (int i = ILS.lengthTL - 2; i >= 0; i--)
            {
                if (course == ILS.TabuListSR[i])
                {
                    for (int j = i + 1; j < ILS.lengthTL - 1; j++)
                    {
                        ILS.TabuListSR[j] = ILS.TabuListSR[j + 1];
                    }
                    return true;
                }
                else { ILS.TabuListSR[i + 1] = ILS.TabuListSR[i]; }
            }
            ILS.TabuListSR[0] = course;
            return false;
        }

        public void ChangeTimeslotMutation()//ref bool KaPermiresim)
        {
            Random random = new Random();

            Solution newSolution = new Solution();

            Solution newSolutionTest = new Solution();

            newSolution = newSolution.Copy(assignments);
            Assignment candidateAssginment = null;

            int day = random.Next(days);
            //int t1 = newSolution.assignments.Count();   
            var assignmentss = newSolution.assignments.Where(p => !Instance.FixedAssignments.Any(p2 => p2.course.Id == p.course.Id));
            int assignCount = assignmentss.Count();
            int rnd = random.Next(assignCount);

            while (IsTabu(assignmentss.ElementAt(rnd).course.Id))//, Rezultati.modesTabuListLen, Rezultati.dcTN1))// 
            {
                rnd = random.Next(assignCount);
            }

            //int t11 = assignmentss.Count();
            var assignment = assignmentss.ElementAt(rnd);

            int timeslot = random.Next(periods_per_day - assignment.course.GetPeriods() + 1);

            var courseGetPeriods = assignment.course.GetPeriods(); //cr
            bool kaPermire = false;

            for (int i = 0; i < periods_per_day - courseGetPeriods; i++) //cr
            {//cr
                timeslot = i;//cr
                if (day != assignment.timeslot.Day)
                {
                    ...
                    {
                        newSolutionTest = newSolutionTest.Copy(this.assignments);
                        ...
                        //KaPermiresim = false;
                        if (this.GetScore() > newSolutionTest.GetScore()) Copy(newSolutionTest.assignments);
                        else if (this.GetScore() < newSolutionTest.GetScore()) kaPermire = true;
                    }
                }
            }
            if (kaPermire) ILS.prtIndex = 10;
            else
            {
                if (ILS.prtIndex < 32) ILS.prtIndex++;
                //else ILS.prtIndex = 10;
            }
        }

        public void Tweak()//ref bool KaPermiresim)
        {
            //SwapRoomMutation();//cr
            ChangeTimeslotMutation();//ref KaPermiresim);
        }

        public void TweakSR()//ref bool KaPermiresim)
        {
            SwapRoomMutation();//ref KaPermiresim);//cr
            //ChangeTimeslotMutation();
        }

        public void Perturb()// int perT)
        {
            for (int i = 0; i < ILS.prtIndex; i++)
                if (i % 2 == 0)//Instance.FixedAssignments.Count()>0)
                {                   
                    ChangeTimeslotMutationPer();
                }
                else
                {
                    SwapCourses();
        ...
        public void ChangeTimeslotMutationPer()
        {
            Random random = new Random();
